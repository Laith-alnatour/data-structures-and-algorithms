...
const toLastNames = people => {
  return people.map(user => `${user.firstName} ${user.lastName}`)
};
...
const addPurchases = (arr) => {
  return (arr.length !==0)?arr.reduce((a, b) => a.purchasePrice + b.purchasePrice): 0;
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((acc,val,idx)=> idx+1);
};
...
const returnNames = (arr) => {
  let namesArr =[]
  arr.reduce((acc,val)=> namesArr.push(val.name),{});
  return namesArr;};
...
const reversedString = (str) => {
  return str.split('').reduce((acc,val) => val.concat(acc));
};
