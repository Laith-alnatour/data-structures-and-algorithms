...
const addOne = (arr) => {
  let resultArr1 = [];
  arr.forEach(n => { resultArr1.push(n+1)});
  return resultArr1;
};
...
const addExclamation = (arr) => {
  let resultArr2 = [];
arr.forEach (exclamation => resultArr2.push(`${exclamation}!`));
return resultArr2;
};
...
const allUpperCase = (arr) => {
  let resultArr3 = [];
  arr.forEach(str => resultArr3.push(str.toUpperCase()));
  return resultArr3;
};
...
const greeting = (word) => {
  let uperWord = `${word.toUpperCase()}!`;
  return uperWord;};

const speaker = (words, callback) => {
  let resultArr4 = [];
  words.forEach(anyWord => resultArr4.push(callback(anyWord)));
  return resultArr4;};
...
const addNumbers = (num, arr, times, callback) => {
  let i =0;
  while (i<times){ callback(arr,num); i++}
...
const createList = (availableit) => {

  let availableList = [], NotavailableList=[];

  availableit.forEach( item => item.available?availableList.push(item.name):NotavailableList.push(item.name))
