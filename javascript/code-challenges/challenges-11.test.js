...
function transformToLis(obj){
  // Solution code here...
  let a =[], values = Object.values(obj), keys = Object.keys(obj);
  for (let i=0; i<values.length;i++){
    a.push(`<li>${keys[i]}: ${values[i]}</li>`)};
  return a;
};
...
const count = (target, input) => {
  // Solution code here...
  return input.length!==0?input.map(arr => arr.filter(nestArr=> nestArr===target).length).reduce((a,b) => a+b):0;
...
const totalSum = (input) => {
  // Solution code here...
  return input.map(arr => arr.reduce((a,b)=>a+b)).reduce((a,b) => a+b);
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(nestArr => nestArr.map(num => {if (num%5===0) return num})
    .filter(item=> typeof item==="number").map(dfive=>Math.pow(2,dfive)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let names='';
  data.forEach((char,idx) => { return (['male','female'].includes(char.gender))? names+=`${char.name}${idx<data.length-1?' and ':''}`:''});
  return names;
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortest = data[0];
  data.forEach(char=> {return parseInt(char.height)<parseInt(shortest.height)?shortest={height:char.height, name:char.name}:''});
